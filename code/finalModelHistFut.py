...
# back percent list
for i in range(len(backTempPercList)):
    if isWithinThreshold(percent, backTempPercList[i], bLowerVal):
    #if dailyHistMeans5[season][i][j] < hYearlyPercentiles[season][i]:
        tempCopy.append(backTempPercList[i])
        dewCopy.append(backDewPercList[i])
        yearCopy.append(backYearPercList[i])
backTempPercList = tempCopy
backDewPercList = dewCopy
backYearPercList = yearCopy
tempCopy = []
dewCopy = []
yearCopy = []
